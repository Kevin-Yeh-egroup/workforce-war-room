...
function personCard(person) {
  ...
  return `<article class="card person-card">
    <div class="person-head">
      <div>
        <div class="person-name-line"><h3>${escapeHtml(person.displayName || '匿名成員')}</h3><span>${escapeHtml(maskedEmail)}</span></div>
    ...
    <div class="kv compact-kv">
      ...
      <div class="k">資料範圍</div><div class="v">姓名與 Email 均已遮罩；不含電話、完整 Email、居住地、備註、工時與薪資狀態</div>
    </div>
...
async function main() {
  ...
  document.getElementById('metrics').innerHTML = [
    metric('去識別成員', counts.total ?? people.length, '遮罩姓名與 Email'),
  ...
  const render = () => {
    ...
    const filtered = people.filter((person) => {
      ...
      return !query || [person.displayName, person.maskedEmail, person.slug].some((value) => String(value || '').toLowerCase().includes(query));
    });
